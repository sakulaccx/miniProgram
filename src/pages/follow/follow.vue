          ...
          <div class="item-box-left">
            <div class="item-box-top">
              ...
              <div class="item-info info-price" v-else>
                ￥{{item.lowestPrice}}
                <div :class="['inner-text',{'green-text': item.priceColor === 0, 'red-text': item.priceColor === 1}]">{{item.priceRang}}</div>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    getLowerset () {
      return (_obj) => {
        console.log(`已降价￥` + Math.abs(_obj.historyPrice - _obj.lowestPrice))
        if ((_obj.historyPrice - _obj.lowestPrice) >= 0) {
          return '已降价￥' + Math.abs(_obj.historyPrice - _obj.lowestPrice)
        } else {
          return '已涨价￥' + Math.abs(_obj.historyPrice - _obj.lowestPrice)
        }
      }
    }
  },
      ...
      this.$fly.post('/attention/getList', {
        ...
        if (res.code === '0' && res.data && res.data.length > 0) {
          ...
          res.data.forEach((v, i) => {
            ...
            this.interestList.push({...v, ...{type: 0, priceRang: priceRang, priceColor: priceColor}})
          })
        } else {
          wx.showToast({
            title: '暂无关注',
